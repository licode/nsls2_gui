...
def run_dpc(filename, i, j, ref_fx=None, ref_fy=None,
            start_point=[1, 0],
            pixel_size=55, focus_to_det=1.46e6, dx=0.1, dy=0.1,
...
def main(file_format='SOFC/SOFC_%05d.tif',
    ...
    mid_col = (np.floor(cols / 2.0) + 1)
    mid_row = (np.floor(rows / 2.0) + 1)

    for i in range(rows):
        for j in range(cols):
            kappax = 2 * np.pi * (j + 1 - mid_col) / (cols * dx)
            kappay = 2 * np.pi * (i + 1 - mid_row) / (rows * dy)
            if kappax == 0 and kappay == 0:
                c[i, j] = 0
            else:
                c[i, j] = -1j * (kappax * tx[i][j] + w * kappay * ty[i][j]) / (kappax ** 2 + w * kappay ** 2)
